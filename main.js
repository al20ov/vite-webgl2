...
const obj = await load("assets/models/extremesphere.obj", OBJLoader);
...
function getMVP() {
  ...
  // const camPos = vec3.fromValues(Math.sin(angle / 10.0) * 10.0, 0.0, 5.0);
  const camPos = vec3.fromValues(0.0, 0.0, 2.0);

  const model = mat4.create();
  mat4.identity(model);
  mat4.fromTranslation(model, vec3.fromValues(0.0, 0.0, 0.0));
  angle += Math.PI / 128;
  ...
  mat4.perspective(projection, 75 * (Math.PI / 180), 16.0 / 9.0, 0.01, 100);
  return [
    camPos,
    model,
    view,
...
function draw() {
  ...
  const drawCall = app.createDrawCall(program, triangleArray)
    ...
    .uniform("roughness", 0.3)
    .uniform("ao", 0.6)
    .uniform("lightPositions[0]", [
      // With directional lights (like the sun), since the light source is
      // "infinitely" far away, distance is irrelevant and what matters is the
      // orientation of the light source. The light source can be as "close" or
      // far away as you want, it won't change a thing as long as it's not at
      // [0, 0, 0]
      Math.cos(angle) * 0.1,
      Math.sin(angle) * 0.1,
      0.0,
